...
//#define USE_OLD_SCALE_RENDERING
	...
	//display area
	const int strHeight = 12;//strMetrics.height();
	...
	//display labels
	{
		...
		if (sfName)
		{
			std::string sfTitle(sfName);
			if (logScale)
				sfTitle += "[Log scale]";
			//we leave some (vertical) space for the top-most label!
			win->displayText(sfTitle, context.glW-xShift, context.glH-yShift+16, ccGLWindow::ALIGN_HRIGHT | ccGLWindow::ALIGN_VTOP);
		}
		...
		for (vlabelSet::iterator it = drawnLabels.begin(); it != drawnLabels.end(); ++it)
		{
			...
			win->displayText(std::to_string(value), x, y+it->yPos, align);
	...
	if (theScaleElements[0].textDisplayed)
		win->displayText(std::to_string(theScaleElements[0].value), halfW+x-5, y+halfH, ccGLWindow::ALIGN_HRIGHT | ccGLWindow::ALIGN_VMIDDLE);
	...
	for (int i=0;i+1<(int)n;++i)
	{
		...
		if (theScaleElements[i+1].textDisplayed)
		{
			...
			win->displayText(std::to_string(dispValue), halfW+x-5, y+halfH, ccGLWindow::ALIGN_HRIGHT | ccGLWindow::ALIGN_VMIDDLE);
		}
