...
public:
	...
	//! Default constructor
	ccColorScaleElement() : m_relativePos(0.0), m_color(0, 0, 0) {}

	//! Constructor from a (relative) position and a color
	ccColorScaleElement(double relativePos, Color color) : m_relativePos(relativePos), m_color(color) {}
...
class QCC_DB_DLL_API ccColorScale
#else
class ccColorScale
#endif
{
public:
	...
	**/
	...
	/** \param name scale name
		...
		0.0 and 1.0).
	**/
	ccColorScale(std::string name, std::string uuid = "");
