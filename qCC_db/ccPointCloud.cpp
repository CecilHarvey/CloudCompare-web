...
ccPointCloud::ccPointCloud(std::string name) throw()
	: ChunkedPointCloud()
...
ccPointCloud* ccPointCloud::partialClone(const CCLib::ReferenceCloud* selection, int* warnings/*=0*/) const
{
	...
	ccPointCloud* result = new ccPointCloud(getName()+".extract");
			...
			if (sf)
			{
				...
				if (sfIdx>=0) //success
				{
					...
					else
					{
						...
						ccLog::Warning(std::string("[ccPointCloud::partialClone] Not enough memory to copy scalar field: ") + sf->getName());
						if (warnings)
...
const ccPointCloud& ccPointCloud::append(ccPointCloud* addedCloud, unsigned pointCountBefore)
{
	...
	if (fabs(shift[0])+fabs(shift[1])+fabs(shift[1])>0.0)
		ccLog::Warning(std::string("[ccPointCloud::fusion] Global shift information for cloud '") + addedCloud->getName() + "' will be lost!");
	...
	for (c=0;c<childrenCount;++c)
	{
		...
		if (child->isKindOf(CC_MESH)) //mesh
		{
			...
			{
				ccLog::Warning(std::string("[ccPointCloud::fusion] Not enough memory: failed to clone sub mesh: ") + mesh->getName());
			}
		}
...
{
	std::shared_ptr<CCLib::ReferenceCloud> c(CCLib::ManualSegmentationTools::segment(this,minVal,maxVal));

	return (c ? partialClone(c.get()) : 0);
}
...
void ccPointCloud::hidePointsByScalarValue(ScalarType minVal, ScalarType maxVal)
{
	...
	{
		ccLog::Error(std::string("[Cloud ") + getName() + "] Visibility table could not be instantiated!");
		return;
	}
	...
	if (!sf)
	{
		ccLog::Error(std::string("[Cloud ") + getName() + "] Internal error: no activated output scalar field!");
		return;
	}
...
ccGenericPointCloud* ccPointCloud::createNewCloudFromVisibilitySelection(bool removeSelectedPoints)
{
	if (!isVisibilityTableInstantiated())
	{
		ccLog::Error(std::string("[Cloud ") + getName() + "] Visibility table not instantiated!");
		return 0;
	}
...
void ccPointCloud::unrollOnCylinder(double radius, CCVector3* center, unsigned char dim/*=2*/, CCLib::GenericProgressCallback* progressCb/*=NULL*/)
{
	...
	if (progressCb)
	{
		...
		char buf[256];
		sprintf(buf, "Number of points = %u\n", numberOfPoints);
...
void ccPointCloud::unrollOnCone(double baseRadius, double alpha_deg, const CCVector3& apex, unsigned char dim/*=2*/, CCLib::GenericProgressCallback* progressCb/*=NULL*/)
{
	...
	if (progressCb)
	{
		...
		char buf[256];
		sprintf(buf, "Number of points = %u", numberOfPoints);
		progressCb->setInfo(buf);
		progressCb->start();
	}
