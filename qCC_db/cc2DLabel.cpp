...
cc2DLabel::cc2DLabel(std::string name/*="label"*/)
	: ccHObject(name.empty() ? "label" : name)
	, m_showFullBody(true)
	, m_dispIn3D(true)
	, m_dispIn2D(true)
{
...
static void replaceString(std::string& str, const std::string &findstr, const std::string &substr)
{
	size_t index = 0;
	while (true) {
		...
		str.replace(index, findstr.length(), substr);
...
std::string cc2DLabel::getName() const
{
	std::string processedName = m_name;

	size_t count = m_points.size();
	if (count>0)
	{
		replaceString(processedName, "pt_0_idx", std::to_string(m_points[0].index));
		if (count>1)
		{
			replaceString(processedName, "pt_1_idx", std::to_string(m_points[1].index));
			if (m_points[0].cloud)
				replaceString(processedName, "pt_0_cloud_id", std::to_string(m_points[0].cloud->getUniqueID()));
			if (m_points[1].cloud)
				replaceString(processedName, "pt_1_cloud_id", std::to_string(m_points[1].cloud->getUniqueID()));
			if (count>2)
			{
				replaceString(processedName, "pt_2_idx", std::to_string(m_points[2].index));
				if (m_points[2].cloud)
					replaceString(processedName, "pt_2_cloud_id", std::to_string(m_points[2].cloud->getUniqueID()));
			}
		}
	}
...
std::list<std::string> cc2DLabel::getLabelContent(int precision)
{
	...
	case 1: //point
		{
			...
			//			std::string coordStr = ("P#%0: (%1;%2;%3)").arg(pointIndex).arg(P->x,0,'f',precision).arg(P->y,0,'f',precision).arg(P->z,0,'f',precision);
			body << coordStr;
...
bool cc2DLabel::acceptClick(int x, int y, int button)
{
...
//unit point marker
static std::shared_ptr<ccSphere> c_unitPointMarker(0);
    ...
    case 1:
		{
			//display point marker as spheres
			{
				...
				{
					c_unitPointMarker = std::make_shared<ccSphere>((PointCoordinateType)1.0f,(const ccGLMatrix*)NULL,"PointMarker",12);
			...
			if (m_dispIn3D && !pushName) //no need to display label in point picking mode
			{
				...
				for (unsigned j=0; j<count; j++)
				{
					...
					char title[256];
					sprintf(title, "P#%d", m_points[j].index);
#if 0 // TODO
